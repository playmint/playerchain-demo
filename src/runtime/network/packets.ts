...
function createHashFunction() {
    ...
    if (!globalThis.process?.versions?.node) {
        ...
        return async (seed: any, opts: any = {}) => {
    ...
    return async (seed: any, opts: any = {}) => {
        ...
        if (seed) {
            ...
            if (bytes) {
                return Buffer.from(value.digest(encoding), encoding);
            }
...
export const PACKET_SPEC: PacketSpec = {
    type: { bytes: 1, encoding: 'number', default: 0 },
...
export const validateMessage = (o: object, constraints: Constraints) => {
    ...
    if (unknownKeys.length) {
        throw new Error(`unexpected keys [${unknownKeys}]`);
    }
...
export const decode = (buf: Buffer) => {
    ...
    for (const [k, spec] of Object.entries(PACKET_SPEC)) {
        ...
        try {
            ...
            if (k === 'message' && isEncodedAsJSON(o as any)) {
                try {
                    value = JSON.parse(value.toString());
                } catch {
                    console.error('failed to parse JSON', value.toString());
                }
            }

            o[k] = value;
        } catch (err) {
            console.error('failed to decode', k, err);
...
export class Packet {
    ...
    constructor(options: Record<string, any> = {}) {
        for (const [k, v] of Object.entries(PACKET_SPEC)) {
            this[k] =
                typeof options[k] === 'undefined' ? v.default : options[k];
    ...
    static async encode(p): Promise<Uint8Array> {
        ...
        if (p.message?.length > Packet.maxLength) {
            throw new Error('ETOOBIG');
        }
