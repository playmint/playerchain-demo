...
export class Cache {
     ...
     *
     */
    async summarize(prefix = '', predicate = (_o: any) => false) {
        // each level has 16 children (0x0-0xf)
        const children: any[] = new Array(16).fill(null).map((_) => []);
    ...
    static encodeSummary(summary) {
        ...
        const prefixBin = Buffer.alloc(
            1 + Math.ceil(summary.prefix.length / 2),
        );
    ...
    static decodeSummary(bin) {
        ...
        while (o < bin.length) {
            buckets[bin.readUint8(o++)] = bin
                .slice(o, (o += Cache.HASH_SIZE_BYTES))
                .toString('hex');
        }
