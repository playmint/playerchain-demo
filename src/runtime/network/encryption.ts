...
export class Encryption {
    ...
    add(publicKey, privateKey) {
        if (!publicKey) {
            throw new Error('encryption.add expects publicKey');
        }
        if (!privateKey) {
            throw new Error('encryption.add expects privateKey');
        }
    ...
    seal(message, v) {
        ...
        return sodium.crypto_box_seal(
            toUint8Array(Buffer.concat([sig, ct])),
            pk,
        );
    }
}
